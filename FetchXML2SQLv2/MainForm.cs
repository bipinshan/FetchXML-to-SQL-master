...
namespace FetchXML2SQLv2
{
    ...
    public partial class MainForm : Form
    {
        ...
        void Button1Click(object sender, EventArgs e)
        {
            ...
            fetchXMLBox.Text=fetchXMLBox.Text.Replace("\\n","").Replace("\\","");
            ...
            foreach (XmlNode linkEntityFilter in xmldoc.DocumentElement.SelectNodes("/fetch/entity/link-entity/filter"))
            {
                if (linkEntityFilter.HasChildNodes)
                {
                    for (int i = 0; i < linkEntityFilter.ChildNodes.Count; i++)
                    {
                        ...
                        {
                            linkEntitiesFiltersList.Add(new linkEntitiesFilters(linkEntityFilter.ParentNode.Attributes["name"].InnerText, linkEntityFilter.Attributes["type"].InnerText.ToUpper(), linkEntityFilter.ChildNodes[i].Attributes["attribute"].InnerText, convertOperator, linkEntityFilter.ChildNodes[i].Attributes["value"].InnerText.Replace("{", "").Replace("}", "")));
                        }
                        else
            ...
            foreach (XmlNode mainFilter in xmldoc.DocumentElement.SelectNodes("/fetch/entity/filter"))
            {
                if (mainFilter.HasChildNodes)
                {
                    for (int i = 0; i < mainFilter.ChildNodes.Count; i++)
                    {
                        ...
                        {
                            mainFiltersList.Add(new mainFilters(mainFilter.ParentNode.Attributes["name"].InnerText, mainFilter.Attributes["type"].InnerText.ToUpper(), mainFilter.ChildNodes[i].Attributes["attribute"].InnerText, convertOperator, mainFilter.ChildNodes[i].Attributes["value"].InnerText.Replace("{","").Replace("}","")));
                        }
                        else
